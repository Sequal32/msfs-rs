...
pub fn get_name_of_named_variable(id: i32) -> Option<String> {
    unsafe {
        let string = sys::get_name_of_named_variable(id);
        Some(std::ffi::CStr::from_ptr(string).to_str().ok()?.to_owned())
    }
}
